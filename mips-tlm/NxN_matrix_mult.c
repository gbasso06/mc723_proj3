...
volatile int still_working1 = 1;
volatile int still_working2 = 1;
...
//  int matriz2[N][N];
  ...
  int index_processor;
  // pega um indice de processador (0 ou 1)
  AcquireLock();
  index_processor = n_processors++;
  //printf("%d \n", index_processor);
  ReleaseLock();
  
//  printf("index processor: %d\n", index_processor);
// srand(time(NULL));
/*
  if (index_processor == 0){
    // se for o primeiro processador, randomiza a primeira matriz.
    for (i = 0 ; i < N; i++){
      for (j = 0; j < N; j++){
        int r = rand();
        matriz1[i][j] = r;
  ...
  still_working1 = 0;
  ReleaseLock();
  printf("randomizou matriz 1\n");
  } else {
    // senão, randomiza a segunda.
    for (i = 0 ; i < N; i++){
      for (j = 0; j < N; j++){
        int r = rand();
        matriz2[i][j] = r;
  ...
  still_working2 = 0;
  ReleaseLock();
  printf("randomizou matriz 2\n");
  }
*/
  //agora multiplica as matrizes:
  //while (still_working1 || still_working2);
  //AcquireLock();
  //still_working1 = still_working2 = 1;
  //ReleaseLock();
  if (index_processor == 0){
    // se for o primeiro processador, pega a primeira metade da multiplicação das matrizes
    // e calcula
    for(i = 0; i < N/2; i++){
      for(j = 0; j < N; j++){
        result[i][j] = 0;
    ...
    // se for o segundo processador, pega a segunda metade da multiplicação das matrizes
    // e calcula
  } else {
    for(i = N/2; i < N; i++){
      for(j = 0; j < N; j++){
  ...
  // enquanto os dois não acabam., espera aqui
  while(still_working2 == 1 || still_working1 == 1) printf("%d\t%d\n", still_working1, still_working2);
  //while(still_working1 || still_working2);
  //depois dos dois acabarem, o primeiro imprime as matrizes e os resultados;
